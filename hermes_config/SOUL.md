...
### /ratelimit
Run the proxy status tool:
python3 /app/bot/telegram_bot.py proxy-status
Parse the JSON output and reply with a Telegram message in this exact format:

🛡️ *Rate-Limit Proxy — Live Status*

`upstream: <upstream value>`
`cap: <rpm_cap>/min per key`

*Per-key windows (last 60s):*
<for each key in per_key_rpm: `key ...XXXXXXXX  →  N req/min`>
(If per_key_rpm is empty: "No LLM calls made yet")

*Retry config:* max=<max_retries> backoff=<base_delay_s>s→300s

*Action hints:*
— If any key shows > 250 req/min → rate limit imminent; Blink will throttle automatically.
— If proxy is DOWN → hermes-agent is hitting DO Inference directly; 429s will not be absorbed.
— To check if the proxy is actually intercepting calls: watch `docker compose logs -f hermes 2>&1 | grep "[proxy]"` on VPS.

If the tool returns an error or the proxy is not running, reply:
"⚠️ Rate-limit proxy is DOWN. Run `docker exec hermes supervisorctl status rate-limit-proxy` on VPS to diagnose. All DO Inference calls are going direct — 429s will not be absorbed."
...
Backoff: exponential with jitter (15s base, 300s max) — proxy absorbs 429s before hermes-agent sees them
