...
class AgenticLoop:
    ...
    def __init__(self, task: str, workdir: str, model: str = "",
        ...
        # API config — prefer OPENAI_BASE_URL (points to rate-limit proxy at
        # localhost:11434) so all calls are rate-limited and model-switched
        # correctly. Fall back to DO_INFERENCE_BASE_URL only when the proxy
        # is explicitly disabled. FIX-BUG2B: was reading DO_INFERENCE_BASE_URL
        # which is always the real DO endpoint, bypassing the proxy entirely.
        self.api_key = (
