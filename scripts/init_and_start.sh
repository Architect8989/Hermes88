...
# Resolve per-model keys.
# FIX-14: Keys previously all collapsed to the same value when only
# DO_INFERENCE_API_KEY was set — fallback switched model but hit the same
# rate-limit window (same key). Now each key is resolved independently:
#   DO_KEY_DEEPSEEK → own value, or DO_INFERENCE_API_KEY
#   DO_KEY_KIMI     → own value ONLY (no deepseek fallback — different quota)
#   DO_KEY_QWEN     → own value ONLY (no deepseek fallback — different quota)
# If you only have one key, set it as DO_INFERENCE_API_KEY AND also add it
# as DO_KEY_KIMI and DO_KEY_QWEN in .env — each model needs its own .env line
# to get genuine rate-limit isolation on DO Inference.
DO_KEY_DEEPSEEK="${DO_KEY_DEEPSEEK:-${DO_INFERENCE_API_KEY}}"
DO_KEY_KIMI="${DO_KEY_KIMI:-}"
DO_KEY_QWEN="${DO_KEY_QWEN:-}"
...
echo "       DO_KEY_KIMI     → kimi-k2.6         : $([ -n "${DO_KEY_KIMI}" ] && echo SET || echo "MISSING (tier will be skipped)")"
echo "       DO_KEY_QWEN     → qwen3.5-397b-a17b : $([ -n "${DO_KEY_QWEN}" ] && echo SET || echo "MISSING (tier will be skipped)")"
...
JCODE_MODEL_VAL="deepseek-v4-pro"
...
export JCODE_API_KEY="${DO_KEY_DEEPSEEK}"  # FIX-BUG6: kimi key is empty/403; use deepseek until kimi provisioned
...
export OPENAI_BASE_URL="http://localhost:11434/v1"  # FIX-BUG2: was DO_BASE_URL (direct); Conway/Blink must route via proxy
...
export HERMES_AUXILIARY_BASE_URL="http://localhost:11434/v1"  # FIX-BUG4: was DO_BASE_URL (direct); auxiliary must route via proxy
...
export OPENROUTER_BASE_URL="http://localhost:11434/v1"  # FIX-13: via rate-limit proxy
# FIX-1: Export per-model keys so the rate-limit proxy can route each fallback
# model with its own independent Authorization header — not the exhausted deepseek key.
export DO_KIMI_API_KEY="${DO_KEY_KIMI}"
export DO_QWEN_API_KEY="${DO_KEY_QWEN}"
echo "[env] Per-model keys exported: DO_KEY_DEEPSEEK (primary) / DO_KEY_KIMI / DO_KEY_QWEN"
...
# ── Resolve per-model keys with full fallback chain ────────────────────────
# Priority: individual per-model key > DO_INFERENCE_API_KEY (legacy) > "".
# The shell layer above already resolved and exported DO_KEY_* using the same
# logic, so os.environ.get() should always find them — but we use .get() here
# defensively so a missing export never raises KeyError and kills the script
# (init_and_start.sh has set -e; any uncaught exception would stop supervisord).
_legacy = os.environ.get('DO_INFERENCE_API_KEY', '')
do_key_deepseek = os.environ.get('DO_KEY_DEEPSEEK') or _legacy
do_key_kimi     = os.environ.get('DO_KEY_KIMI') or ''   # NO fallback to deepseek — different quota/model scope
do_key_qwen     = os.environ.get('DO_KEY_QWEN') or ''   # NO fallback to deepseek — different quota/model scope

if not do_key_deepseek:
    print("[FATAL] No DO Inference API key found. Set DO_KEY_DEEPSEEK or DO_INFERENCE_API_KEY.", file=sys.stderr)
    sys.exit(1)

using_3_keys = (
    bool(os.environ.get('DO_KEY_DEEPSEEK')) and
    bool(os.environ.get('DO_KEY_KIMI')) and
    bool(os.environ.get('DO_KEY_QWEN'))
)
key_mode = "3 per-model keys (isolated quotas)" if using_3_keys else "single-key legacy mode (DO_INFERENCE_API_KEY → all models)"
print(f"[hermes-agent] Key mode: {key_mode}")

lines = [
    "HERMES_HOME=/data/.hermes",
    # Per-model keys — written so config.yaml ${DO_KEY_*} template vars expand correctly
    f"DO_KEY_DEEPSEEK={do_key_deepseek}",
    f"DO_KEY_KIMI={do_key_kimi}",
    f"DO_KEY_QWEN={do_key_qwen}",
    # Legacy alias — always set to deepseek key for backward compat
    f"DO_INFERENCE_API_KEY={do_key_deepseek}",
    f"DO_INFERENCE_BASE_URL={do_base_url}",
    f"DO_BASE_URL={do_base_url}",
    f"HERMES_MODEL={os.environ.get('HERMES_MODEL', 'deepseek-v4-pro')}",
    f"OPENCLAUDE_MODEL={os.environ.get('OPENCLAUDE_MODEL', 'deepseek-r1-distill-llama-70b')}",
    f"JCODE_MODEL={os.environ.get('JCODE_MODEL', 'kimi-k2.6')}",
    f"DO_FALLBACK_MODEL={os.environ.get('DO_FALLBACK_MODEL', 'deepseek-r1-distill-llama-70b')}",
    # OPENAI_* → route through proxy so Conway/Blink subprocess calls are rate-limited
    # FIX-BUG2: was do_base_url (direct DO endpoint) — subprocesses bypassed proxy entirely
    f"OPENAI_API_KEY={do_key_deepseek}",
    "OPENAI_BASE_URL=http://localhost:11434/v1",
    f"OPENAI_MODEL={os.environ.get('OPENCLAUDE_MODEL', 'deepseek-r1-distill-llama-70b')}",
    "CLAUDE_CODE_USE_OPENAI=1",
    f"ANTHROPIC_API_KEY={do_key_deepseek}",
    # Auxiliary compression — route through proxy so quota is properly managed
    # FIX-BUG4: was do_base_url (direct DO endpoint) — auxiliary calls bypassed proxy
    "HERMES_AUXILIARY_PROVIDER=openai",
    "HERMES_AUXILIARY_BASE_URL=http://localhost:11434/v1",
    f"HERMES_AUXILIARY_API_KEY={do_key_kimi}",
    "HERMES_AUXILIARY_MODEL=kimi-k2.6",
    ...
    f"GITHUB_PAT={os.environ.get('GITHUB_PAT', '')}",
    ...
    # hermes-agent routes via "openrouter"; OPENROUTER_* redirects to DO Inference.
    f"OPENROUTER_API_KEY={do_key_deepseek}",
    "OPENROUTER_BASE_URL=http://localhost:11434/v1",
    # FIX-1: Per-model keys written into gateway.yaml so the proxy can read them
    # via env and route each fallback model with its own DO quota key.
    f"DO_KIMI_API_KEY={do_key_kimi}",
    f"DO_QWEN_API_KEY={do_key_qwen}",
]
p.write_text("\n".join(lines) + "\n")
print(f"[hermes-agent] .env written ({len(lines)} vars). TELEGRAM_ALLOWED_USERS={'set' if telegram_chat_id else 'not set — bot open to all'}")
PYEOF
