...
# FIX-2: hermes-agent module check now uses `hermes_cli` (correct package
#        module name). hermes-agent installs modules: hermes_cli, hermes_constants,
#        run_agent, gateway — NOT `hermes_agent`. CLI check via shutil.which is
#        preferred; hermes_cli import is the secondary confirmation.
# ─────────────────────────────────────────────────────────────────────────────
echo ""
echo "[check] Agent and tool availability:"
python3 -c "import hermes_cli; print('  hermes-agent (hermes_cli module): OK')" 2>/dev/null \
    || { hermes-agent --version 2>/dev/null | head -1 | sed 's/^/  hermes-agent CLI: /'; } \
    || echo "  hermes-agent: not installed — gateway/run.py fallback will be used (OK)"
# openclaude gRPC: stubs are generated by Dockerfile into /app/skills/openclaude_grpc/
python3 -c "
import sys; sys.path.insert(0,'/app/skills/openclaude_grpc')
try:
    import grpc, openclaude_pb2
    print('  openclaude-grpc stubs: OK (Python gRPC client ready)')
except ImportError as e:
    print('  openclaude-grpc stubs: NOT READY —', e)
" 2>/dev/null || echo "  openclaude-grpc: stubs not generated yet (Dockerfile generates them at build)"
# Check if openclaude Node/Bun source is cloned
[ -f /opt/openclaude/scripts/start-grpc.ts ] \
    && echo "  openclaude (Node/Bun): /opt/openclaude source present" \
    || echo "  openclaude (Node/Bun): not cloned — Python gRPC server fallback active"
openclaw   --version 2>/dev/null | head -1 | sed 's/^/  openclaw:   /' || echo "  openclaw: not installed (non-critical)"
jcode      --version 2>/dev/null | head -1 | sed 's/^/  jcode:      /' || echo "  jcode: not found in PATH"
