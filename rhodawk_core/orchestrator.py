...
Wraps patterns from BerriAI/litellm for unified API routing across
multiple providers with automatic failback.
...
class Orchestrator:
    """
    Unified LLM orchestrator with multi-model routing and automatic failover.

    Supports task-type-based model selection (reasoning, coding, scaffolding,
    embedding, fast), automatic failover chains, exponential backoff on errors,
    request caching, and comprehensive request logging.
    """

    # Task type to model mapping (primary -> fallback chain)
    DEFAULT_MODEL_CHAINS = {
        "reasoning": ["deepseek-r1", "kimi-k2.6", "qwen3-235b-a22b"],
        "coding": ["deepseek-v4-0324", "kimi-k2.6", "qwen3-235b-a22b"],
        "scaffolding": ["kimi-k2.6", "deepseek-v4-0324", "qwen3-235b-a22b"],
        "fast": ["kimi-k2.6", "deepseek-v4-0324"],
        "embedding": ["text-embedding-3-small"],
        "general": ["kimi-k2.6", "deepseek-v4-0324", "qwen3-235b-a22b"],
    }
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        # API Configuration
        self.base_url = config.get("base_url", os.environ.get(
            "DO_INFERENCE_BASE_URL", "https://inference.do-ai.run/v1"
        ))
        self.api_key = config.get("api_key", os.environ.get(
            "DO_INFERENCE_API_KEY", ""
        ))

        # Model chains (task_type -> ordered list of models to try)
    ...
    def _init_default_models(self):
        """Initialize default model configurations for DO Inference."""
        defaults = [
            ModelConfig(
                name="deepseek-r1",
    ...
    def route_request(self, messages: List[Dict[str, str]],
        ...
        Returns:
            Dict with keys: content, model, tokens_used, latency_ms, cached
        """
        # Check cache first
        if cache_key:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for r in entries:
            if r.model not in model_usage:
                model_usage[r.model] = {"requests": 0, "tokens": 0, "failures": 0}
